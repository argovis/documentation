...
----------------
 ...
 - When it's time to renew certs, you'll receive an email from OIT / Sectigo. Use the included links to log in to Sectigo and request renewals through their dashboard.
   ...
   - You can also be a bit more proactive about this: `log into sectigo <https://hard.cert-manager.com/customer/cuboulder/ssl/login>`_ any time before certs expire, request renewals for the frontend, core API, drifter API and keygen certs, and fill out `this form <https://oit.colorado.edu/services/it-security/request-ssl-certificate/webform/request-ssl-certificate>`_ or open an OIT ticket confirming these have no personal or confidential info associated with them and can they please approve the renewal request.
